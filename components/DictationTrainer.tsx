...
export default function DictationTrainer() {
  const [index,setIndex]=useState(0);const [answer,setAnswer]=useState("");const [checked,setChecked]=useState(false);const [sync,setSync]=useState("");const current=lines[index];const accuracy=useMemo(()=>checked?compare(current,answer):0,[answer,checked,current]);
  function speak(rate=.88){if(!("speechSynthesis" in window))return;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(current);u.lang="en-US";u.rate=rate;window.speechSynthesis.speak(u)}
