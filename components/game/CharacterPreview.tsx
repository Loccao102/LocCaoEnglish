...
import { expressions, personalityFor, type Expression } from "@/lib/game/personalities";
...
export default function CharacterPreview({ id }: { id: string }) {
  ...
  const [expression,setExpression]=useState<Expression>(personalityFor(id).expression);
  useEffect(()=>{
    let cancelled=false;let instance:ModelPreview|null=null;setError(false);setPose("idle");setExpression(personalityFor(id).expression);
