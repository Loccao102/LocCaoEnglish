...
function AdventureRuntime({ adventure, initialMap }: { adventure: ReturnType<typeof useAdventure>; initialMap: boolean }) {
  ...
  return <div className="adventure-game">
    ...
    <div className="game-world-ui" inert={!enabled} hidden={title}>
      ...
      <nav className="game-toolbar" aria-label="Adventure tools"><Link className="fair-world-link" href="/journey">♡ My journey</Link><Link className="fair-world-link" href="/festival">✿ Friendship Fair</Link><button onClick={() => setPanel("journal")}><span>▤</span>Journal<kbd>J</kbd></button><button onClick={() => setPanel("map")}><span>⌘</span>Map<kbd>M</kbd></button><button onClick={() => setPanel("bag")}><GameArt id="satchel"/>Bag<kbd>B</kbd></button></nav>
    </div>
