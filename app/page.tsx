    ...
    <div className="page-wrap">
      <header className="topbar">
        <div>
          ...
          <p>Every answer changes your build. Weak skills create recovery quests; your goal decides where today&apos;s run begins.</p>
        </div>
        <div className="top-actions">
          <span className="streak">🔥 keep the run alive</span>
          <Link className="button ghost" href="/onboarding">Tune build</Link>
          <Link className="button primary" href="/learn">Continue journey →</Link>
        </div>
      </header>
      ...
      <section className="hero-grid">
        <article className="continue-card">
          <div className="continue-copy">
            ...
            <div className="mission-meta"><span>✦ 4 stages</span><span>◷ 15 min</span><span>+150 XP</span></div>
        ...
        <article className="panel quest-card">
          <div className="section-heading"><div><span className="eyebrow">DAILY RUN</span><h2>One short run, real progress</h2></div><span className="xp-pill">adaptive</span></div>
          <p className="muted">The director mixes your player goal, weakest skills and due reviews. Finish this before free-playing other modes.</p>
          <div className="quest-list"><div className="quest"><span className="check">1</span><span>Goal quest</span><small>your build</small></div><div className="quest"><span className="check">2</span><span>Recover a weak skill</span><small>adaptive</small></div><div className="quest"><span className="check">3</span><span>Bank the XP</span><small>streak</small></div></div>
      ...
      <section className="panel">
        <div className="section-heading"><div><span className="eyebrow">WORLD SELECT</span><h2>Choose where to play next</h2></div><Link href="/courses" className="text-link">See campaigns →</Link></div>
        <div className="quick-games">{worlds.map((world) => <Link className="quick-game" href={world.href} key={world.title}><span>{world.icon}</span><div><small>{world.tag}</small><strong>{world.title}</strong><small>{world.copy}</small></div><b>→</b></Link>)}</div>
      </section>

      <SkillMap />

      <section className="split-grid">
        <article className="panel"><div className="section-heading"><div><span className="eyebrow">ARCADE</span><h2>Quick XP runs</h2></div><Link href="/games" className="text-link">All games →</Link></div><div className="quick-games">{games.slice(0, 4).map((game) => <Link className="quick-game" href={game.href} key={game.slug}><span>{game.icon}</span><div><strong>{game.name}</strong><small>{game.skill} · {game.minutes} min</small></div><b>→</b></Link>)}</div></article>
        <article className="panel"><div className="section-heading"><div><span className="eyebrow">HOW THE GAME LEARNS YOU</span><h2>Mistakes become content</h2></div></div><div className="quest-list"><div className="quest"><span className="check">①</span><span>Play any quest or mini-game</span><small>evidence</small></div><div className="quest"><span className="check">②</span><span>Weak concepts enter Recovery</span><small>SRS</small></div><div className="quest"><span className="check">③</span><span>Daily quests adapt automatically</span><small>AI + rules</small></div><div className="quest"><span className="check">④</span><span>Bosses force you to use skills together</span><small>mastery</small></div></div></article>
      </section>
    </div>
  );
}
