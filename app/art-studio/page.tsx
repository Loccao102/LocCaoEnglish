...
export default function ArtStudioPage() {
  return <div className="page-wrap art-studio">
    <header className="art-studio-header">
      ...
      <div className="art-companions" aria-hidden="true"><GameArt id="mam" /><GameArt id="nang" /><GameArt id="may" /><GameArt id="soi" /></div>
    </header>
    <section className="model-collection" aria-labelledby="model-title">
      <div className="model-intro"><div><p className="eyebrow">FROM OUR LITTLE WORLD</p><h2 id="model-title">A world you can walk around.</h2><p>Four animated companions, seven places to discover, little objects and a complete island. These are the models used to build the 3D adventure.</p></div><Link className="button primary" href="/play">Visit the 3D village →</Link></div>
      <div className="model-download-grid">{models.assets.map(model => <a key={model.id} href={model.src} download><span className="model-file">{model.animations.length ? "ANIMATED CHARACTER" : model.id === "village" ? "COMPLETE WORLD" : "WORLD ASSET"}</span><strong>{model.id.replaceAll("-", " ")}</strong><span>{model.animations.length ? "Idle · walk · run · jump · wave · celebrate" : "Reusable 3D model"}</span><small>GLB · {model.bytes > 1048576 ? `${(model.bytes / 1048576).toFixed(1)} MB` : `${Math.round(model.bytes / 1024)} KB`} <b aria-hidden="true">↓</b></small></a>)}</div>
      <p className="model-note">Companions include six animations and articulated joints. The world file contains the scenery; movement, quests and interactions live in the game. <a href="/assets/sunlit-3d/PROVENANCE.md">3D provenance & reuse notes</a> · <a href="/assets/sunlit-3d/manifest.json" download>Model manifest</a></p>
    </section>
