...
func (s *Server) completeBoss(w http.ResponseWriter, r *http.Request) { id,ok:=s.userID(w,r);if!ok{return};var in struct{Score int `json:"score"`};if!decode(w,r,&in){return};state,err:=s.store.CompleteBoss(r.Context(),id,r.PathValue("id"),in.Score);if errors.Is(err,store.ErrNotFound){problem(w,404,"boss not found");return};if err!=nil{problem(w,409,err.Error());return};write(w,200,state) }
