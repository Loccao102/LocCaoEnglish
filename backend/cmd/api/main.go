...
func main() {
	port := env("PORT", "8080")
	st, err := store.New(os.Getenv("DATABASE_URL")); if err != nil { log.Fatalf("initialize store: %v", err) }
	defer st.Close()
	if err := st.EnsureContent(context.Background()); err != nil { log.Fatalf("initialize content store: %v", err) }
	social := realtime.New(os.Getenv("REDIS_URL")); defer social.Close()
	...
	go func() { log.Printf("LocCaoEnglish API listening on :%s (store=%s realtime=%s)", port, st.Mode(), social.Mode()); errCh <- httpServer.ListenAndServe() }()
	select {
	case <-ctx.Done(): log.Printf("shutdown signal received")
	case err := <-errCh: if !errors.Is(err, http.ErrServerClosed) { log.Fatalf("http server: %v", err) }; return
	}
	shutdownCtx, cancel := context.WithTimeout(context.Background(), 10*time.Second); defer cancel()
	if err := httpServer.Shutdown(shutdownCtx); err != nil { log.Printf("graceful shutdown: %v", err) }
}
